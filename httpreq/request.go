...
func New() *SuperAgent {
	...
	debug := os.Getenv("HTTPREQUEST_DEBUG") == "1"
	...
	s := &SuperAgent{
		...
		Debug:             debug,
		CurlCommand:       false,
		logger:            logrus.New(), // .WithFields(logrus.Fields{"pkg": "httpreq"})
		isClone:           false,
	}
...
//
//      httpreq.New().TLSClientConfig(&tls.Config{ InsecureSkipVerify: true}).
...
//
//      httpreq.New().Proxy("http://myproxy:9999").
...
//        Post("http://www.google.com").
//        End()
// https://github.com/beego/beego/blob/12af439a9c487f8ce3c561f4fe669f3ef2b3b0a4/client/httplib/httplib.go
//
...
// For example:
//
//    resp, body, errs := httpreq.New().Get("http://www.google.com").End()
//    if errs != nil {
//      fmt.Println(errs)
//    }
...
// For example:
//
//    func printBody(resp httpreq.Response, body string, errs []error){
//      fmt.Println(resp.Status)
//    }
//    httpreq.New().Get("http://www..google.com").End(printBody)
//
...
func (s *SuperAgent) getResponseBytes() (Response, []byte, []error) {
	...
	if s.Debug {
		...
		s.logger.WithFields(logrus.Fields{"prefix": "[http]"})
		if err != nil {
	...
	if s.CurlCommand {
		...
		s.logger.WithFields(logrus.Fields{"pkg": "curl"}) //SetPrefix("[curl] ")
		if err != nil {
			s.logger.Println("Error:", err)
		} else {
