...
HTTPRequest -- Simplified HTTP client ( inspired by famous SuperAgent lib in Node.js )
...
## Why should you use HTTPRequest?

HTTPRequest makes thing much more simple for you, making http client more awesome and fun like SuperAgent + golang style usage.

This is what you normally do for a simple GET without HTTPRequest:
...
```go
resp, body, errs := httpreq.New().Get("http://example.com/").End()
```
...
req.Header.Set("Notes","HTTPRequest is coming!")
client := &http.Client{}
resp, _ := client.Do(req)
```

Compared to our HTTPRequest version, JSON is for sure a default. So, it turns out to be just one simple line!:
...
Moreover, HTTPRequest also supports callback function. This gives you much more flexibility on using it. You can use it any way to match your own style!
Let's see a bit of callback example:

```go
func printStatus(resp httpreq.Response, body string, errs []error){
  fmt.Println(resp.Status)
}
httpreq.New().Get("http://example.com").End(printStatus)
```
...
```go
httpreq.New().Post("http://example.com/").
  Type("multipart").
  Send(`{"query1":"test"}`).
  End()
```
...
In the case when you are behind proxy, HTTPRequest can handle it easily with Proxy func:

```go
request := httpreq.New().Proxy("http://proxy:999")
...
```go
request := httpreq.New().SetBasicAuth("username", "password")
resp, body, errs := request.Get("http://example-proxy.com").End()
```
...
```go
request := httpreq.New().Timeout(2*time.Millisecond)
resp, body, errs:= request.Get("http://example.com").End()
```
...
```go
resp, bodyBytes, errs := httpreq.New().Get("http://example.com/").EndBytes()
```
...
resp, _, errs := httpreq.New().Get("http://example.com/").EndStruct(&heyYou)
```
...
For debugging, HTTPRequest leverages `httputil` to dump details of every request/response. (Thanks to @dafang)
...
Thanks to @QuentinPerez, we can see even how httpreq is compared to CURL by using `SetCurlCommand`.

## Noted
As the underlying gorequest is based on http.Client in most use cases, httpreq.New() should be called once and reuse gorequest as much as possible.
